...
class UploadQueue extends EventEmitter {
  ...
  /**
   * Approve files for processing.
   * In one-stage mode: approve → extract + upload automatically.
   * In two-stage mode: approve → extract only, then file waits for upload approval.
   */
  approve(filePath) {
  ...
  /**
   * Approve upload for files in two-stage mode (after extraction is done).
   */
  ...
  /**
   * Approve upload for multiple files in two-stage mode.
   */
  ...
  /**
   * Approve all awaiting_upload files (Audio tab bulk action, two-stage).
   */
  approveAllUploads() {
  ...
  requeue(filePath) {
    ...
    // If it had a WAV and we're in two-stage, go to awaiting_upload
    if (row.wav_path && fs.existsSync(row.wav_path) && this.workflow.approvalMode === 'two-stage') {
  ...
  async _processFile(filePath) {
    ...
    try {
      ...
      if (row && row.status === 'uploading' && row.wav_path && fs.existsSync(row.wav_path)) {
        ...
        // Check for existing WAV (dedup: skip extraction if WAV already exists)
        const expectedWav = path.join(
        ...
        if (fs.existsSync(expectedWav)) {
          ...
          this.emit('queue:updated');
          // Skip actual extraction — WAV already exists
        } else {
          // Extract audio
          this.emit('file:extracting', filePath);
          db.updateStatus(filePath, 'extracting');
          this.emit('queue:updated');
          wavPath = await extractAudio(filePath, this.workflow.outputDir || null);
          db.updateStatus(filePath, 'extracting', { wavPath });
        }

        // Two-stage mode: stop after extraction, wait for upload approval
        if (this.workflow.approvalMode === 'two-stage') {
          db.updateStatus(filePath, 'awaiting_upload', { wavPath });
          this.emit('file:awaiting-upload', filePath);
          this.emit('queue:updated');
          return; // Done for now — user must approve upload separately
        }
      }
      ...
      if (currentRow && currentRow.job_id) {
        ...
        jobId = await uploadWithRetry(wavPath, this.workflow.serverUrl, this.globalConfig.maxRetries || 5);
      ...
      // Poll for completion
      const job = await pollJob(jobId, this.workflow.serverUrl, this.globalConfig.pollingInterval || 5000);
