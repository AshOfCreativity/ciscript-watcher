...
const VALID_AUDIO_FORMATS = ['wav', 'flac', 'mp3'];
...
function validateWorkflow(wf) {
  ...
  if (wf.audioFormat && !VALID_AUDIO_FORMATS.includes(wf.audioFormat)) {
    throw new Error(`Workflow "${wf.name}": invalid audioFormat "${wf.audioFormat}" (must be ${VALID_AUDIO_FORMATS.join(', ')})`);
  }
...
module.exports = { loadConfig, saveConfig, WORKFLOW_DEFAULTS, GLOBAL_DEFAULTS, VALID_AUDIO_FORMATS, expandHome };
