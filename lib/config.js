...
function loadConfig(configPath, cliOverrides = {}) {
  ...
  if (configPath) {
    ...
    // Try default location (check userData first for packaged Electron, then app dir)
    let defaultPath;
    try {
      const { app } = require('electron');
      defaultPath = path.join(app.getPath('userData'), 'watcher-config.json');
    } catch {
  ...
  // Resolve dbPath — use userData when running inside packaged Electron
  if (!config.dbPath) {
    let dataRoot;
    try {
      const { app } = require('electron');
    ...
    config.dbPath = path.join(dataRoot, 'watcher.db');
  } else {
